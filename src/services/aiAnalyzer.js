// Enhanced AI Analyzer Service - FIXED VERSION
...
// FIXED: Using currently available Groq models (December 2024)
const PRIMARY_MODEL = 'llama-3.1-8b-instant' // Fast, reliable model with broad API key compatibility
const FALLBACK_MODELS = [
  'gemma2-9b-it', // Alternative model with separate limits
  'llama-3.1-8b-instant', // Fast model that works
  'mixtral-8x7b-32768', // Might work with different requests
  'deepseek-r1-distill-llama-70b' // Available as backup
]
...
class EnhancedAIAnalyzer {
  constructor() {
    ...
    console.log(`🤖 Enhanced AI Analyzer initialized with primary model: ${PRIMARY_MODEL} (switched from rate-limited model)`)
  ...
  // PRIMARY ENTRY POINT FOR CALLERS EXPECTING AI-ENRICHED UPDATES
  ...
  extractContentForAnalysis(update = {}, metadata = {}) {
    const contentPieces = []
    ...
    appendText(update.ai_summary) // previously generated summary
    ...
    // Limit to prevent excessively long payloads
    const MAX_LENGTH = 6000
    return combined.length > MAX_LENGTH ? combined.substring(0, MAX_LENGTH) : combined
  }

  normalizeAnalysisResult(analysis = {}, update = {}, metadata = {}) {
  ...
  buildAnalyzeUpdateResponse(normalized, options = {}) {
    return {
      success: true,
...
Ensure accuracy, professionalism, and focus on actionable business intelligence.`
  ...
  createEnhancedFallbackAnalysis(content, url, metadata = {}) {
    ...
    const fallbackAnalysis = {
      ...
      aiTags: this.generateAITags({ impactLevel, urgency, primarySectors: detectedSectors, area: this.detectRegulatoryArea(content) }, content),
  ...
  generateFallbackImpact(content, impactLevel) {
    ...
    // Find the first complete sentence or meaningful excerpt
    ...
    // Generate appropriate impact statement based on level
  ...
  // FIXED GROQ API REQUEST METHOD WITH BETTER ERROR HANDLING
  ...
  parseAndValidateAIResponse(response, content, url) {
    try {
      ...
      // Apply defaults for missing fields and clean up broken text
